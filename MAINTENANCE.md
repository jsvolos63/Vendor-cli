...
This repo's own health is the shortest thing in this file: 227 tests green,
...
| `npm test` | `node --test` over **nine explicitly named files** — `test/test.mjs`, `test/tooling.test.mjs`, `test/claude-md-sync.test.mjs`, `test/maintenance-sync.test.mjs`, `test/family-liveness.test.mjs`, `test/maintenance-doc-check.test.mjs`, `test/sanitizer-policy-sync.test.mjs`, `test/module-graph.test.mjs`, `test/workflows.test.mjs`. 227 cases, ~18 s. |
...
8. **The family's automation stopped.** Run the monitor by hand before reading
   ...
   off. A PR under "Held" is not a finding — it is there so a hold stays in
   sight until its condition is met.
