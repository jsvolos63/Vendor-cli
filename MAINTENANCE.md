...
| The bump's untrusted steps never hold a write token | `kit-pin-bump.yml`: the job that runs `inputs.install-command` / `vendor-sync-command` / `check-command` is `permissions: contents: read`; only the job that applies the patch, opens the PR and merges has write, and it runs no `npm`/`npx`/`inputs.*` command | **`test/workflows.test.mjs`** ("runs every untrusted command under a read-only token"), each half shown to fail on a mutated copy | a compromised kit commit executing in Monday's bump with `contents: write` in twelve repos at once — the family audit's FAM-1. The split is the boundary; the pin still resolves from HEAD unreviewed. |
